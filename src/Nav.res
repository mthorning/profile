...
module NavItem = {
  ...
  let make = (~page) =>
    <li className="li-hz">
      {switch page {
      | "" => <a className="col-p-hv navbar-item" href="/"> <Icons.Home className="font-3" /> </a>
      | "contact" => <a className="navbar-item border-p-hv" href="#contact"> {page->str} </a>
      | _ => <a className="navbar-item border-p-hv" href=page> {page->str} </a>
      }}
    </li>
}

let pages = list{"", "about", "resume", "contact"}
